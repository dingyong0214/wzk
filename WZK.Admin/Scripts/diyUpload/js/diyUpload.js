...
(function( $ ) {
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
 			if ( typeof opt != "object" ) {
 			    layer.msg("参数错误", { time: 1200, icon: 2 });
				return;	
			}
			...
			if ( !WebUploader.Uploader.support() ) {
			    layer.msg("上传组件不支持您的浏览器", { time: 1200, icon: 2 });
				return false;
       		}
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
				    case 'Q_EXCEED_NUM_LIMIT': text = '上传图片数量超过限制';
					break;
				    case 'F_EXCEED_SIZE': text = '图片大小超过限制!';
					break;
				    case 'Q_EXCEED_SIZE_LIMIT': text = '所有图片总大小超过限制';
					break;
				...
				layer.msg(text, { time: 1200, icon: 2 });
        	});
        }
    });
	...
	function getOption(objId) {
		...
		*/
		return {
			...
			pick:{
				id:objId,
				label: "<i class='icon-plus'></i>  添加图片"
			},
			//类型限制;
			accept:{
				title:"Images",
	...
	function showDiyProgress( progress, $diyBar, text ) {
		...
		setTimeout(function () {
		    $diyBar.fadeOut();
		    if (~text.indexOf("失败")) {
		        var $elem = $diyBar.parent('li');
		        //$diyBar.parent().remove();
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			var div = '<div class="diyButton"> \
						...
						<a class="diyCancelAll btn btn-danger" href="javascript:void(0)">全部取消</a> \
					</div>';

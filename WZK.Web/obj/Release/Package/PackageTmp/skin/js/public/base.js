...
cayiVote.layerMsg = function (msg, time, callback) { //msg消息；time关闭时间(单位为秒)，默认三秒；callback关闭后回调；time和callback没有先后之分
	...
	cayiVote.layerConfirm = function (msg, yesCallback, noCallback) { //msg消息；yesCallback确定按钮回调；noCallback取消按钮回调，返回false时不关闭弹出层
	...
	cayiVote.layerAlert = function (msg, time, callback) { //msg消息；time关闭时间(单位为秒)，可不填，表示不关闭；callback关闭后回调；time和callback没有先后之分
	    if (typeof (time) == "function") {
	        callback = [time, time = callback][0];
	    };
	    layer.open({
	...
	cayiVote.layerPage = function (opt) {
	    opt = opt || {};
	    var arr = ["btn", "yes", "cancel"],
			json = {
			    btn: opt.btn || ['确定', '取消'],
...
$(function () {
    ...
    $(document).ajaxSend(function () {
        layerLoad = cayiVote.loading && layer.load(2, { time: 60 * 1000 });
    });
